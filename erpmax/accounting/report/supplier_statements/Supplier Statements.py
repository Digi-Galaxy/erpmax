...
def execute(filters=None):
    ...
    rows = frappe.db.sql("""SELECT name, supplier, posting_date, due_date,
              base_grand_total, paid_amount, outstanding_amount, status
       FROM `tabPurchase Invoice`
       WHERE {cond} ORDER BY supplier, posting_date""".format(cond=" AND ".join(cond)),
        params, as_dict=True)
    data = [{"name": r.name, "supplier": r.supplier, "posting_date": r.posting_date,
...
def get_columns():
    return [
        {"label": _("Invoice"), "fieldname": "name", "fieldtype": "Link", "options": "Purchase Invoice", "width": 180},
