...
def execute(filters=None):
    ...
    if filters.get("item_name"):
        cond.append("sii.item_name=%(item_name)s")
        params["item_name"] = filters["item_name"]

    rows = frappe.db.sql("""SELECT sii.item_name AS item,
              SUM(sii.qty) AS qty,
              SUM(sii.amount) AS amount,
              SUM(sii.rate) / COUNT(*) AS avg_rate,
              COUNT(DISTINCT si.name) AS invoice_count
       FROM `tabSales Invoice Item` sii
       INNER JOIN `tabSales Invoice` si ON si.name = sii.parent
       WHERE {cond} GROUP BY sii.item_name ORDER BY amount DESC""".format(cond=" AND ".join(cond)),
        params, as_dict=True)
    data = [{"item": r.item, "qty": r.qty or 0,
