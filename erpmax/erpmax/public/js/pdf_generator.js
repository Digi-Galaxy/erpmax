frappe.provide("erpmax.pdf_generator");

frappe.ui.form.on("Sales Invoice", { refresh: add_pdf_button });
frappe.ui.form.on("Purchase Invoice", { refresh: add_pdf_button });
frappe.ui.form.on("Purchase Order", { refresh: add_pdf_button });
frappe.ui.form.on("Purchase Receipt", { refresh: add_pdf_button });
frappe.ui.form.on("Journal Entry", { refresh: add_pdf_button });
frappe.ui.form.on("Payment Entry", { refresh: add_pdf_button });
frappe.ui.form.on("Expense Claim", { refresh: add_pdf_button });
...
function generate_pdf_dialog(frm) {
    frappe.call({
        ...
        args: {
            ...
            filters: { doc_type: frm.doctype, disabled: 0 },
            fields: ["name"],
            order_by: "name asc"
        },
        callback: function (r) {
